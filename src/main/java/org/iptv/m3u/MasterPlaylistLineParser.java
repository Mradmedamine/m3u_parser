...
class MasterPlaylistLineParser implements LineParser {
...

		{
			HANDLERS.put(Constants.TYPE, new AttributeParser<MediaData.Builder>() {
				...
				public void parse(Attribute attribute, MediaData.Builder builder, ParseState state)
					...
					if (type == null) {
						throw ParseException.create(ParseExceptionType.INVALID_MEDIA_TYPE, getTag(),
						        attribute.toString());
					} else {
			...
			HANDLERS.put(Constants.URI, new AttributeParser<MediaData.Builder>() {
				...
				public void parse(Attribute attribute, MediaData.Builder builder, ParseState state)
				        throws ParseException {
					builder.withUri(ParseUtil.decodeUri(ParseUtil.parseQuotedString(attribute.value, getTag()),
					        state.encoding));
				}
			});
			...
			HANDLERS.put(Constants.GROUP_ID, new AttributeParser<MediaData.Builder>() {
				...
				public void parse(Attribute attribute, MediaData.Builder builder, ParseState state)
					...
					if (groupId.isEmpty()) {
						throw ParseException.create(ParseExceptionType.EMPTY_MEDIA_GROUP_ID, getTag(),
						        attribute.toString());
					} else {
			...
			HANDLERS.put(Constants.NAME, new AttributeParser<MediaData.Builder>() {
				...
				public void parse(Attribute attribute, MediaData.Builder builder, ParseState state)
					...
					if (name.isEmpty()) {
						throw ParseException.create(ParseExceptionType.EMPTY_MEDIA_NAME, getTag(),
						        attribute.toString());
					} else {
				...
				public void parse(Attribute attribute, MediaData.Builder builder, ParseState state)
					...
					if (isDefault) {
						if (state.getMaster().isNotAutoSelect) {
							throw ParseException.create(ParseExceptionType.AUTO_SELECT_DISABLED_FOR_DEFAULT, getTag(),
							        attribute.toString());
						}
			...
			HANDLERS.put(Constants.IN_STREAM_ID, new AttributeParser<MediaData.Builder>() {
				...
				public void parse(Attribute attribute, MediaData.Builder builder, ParseState state)
					...
					if (Constants.EXT_X_MEDIA_IN_STREAM_ID_PATTERN.matcher(inStreamId).matches()) {
						...
						throw ParseException.create(ParseExceptionType.INVALID_MEDIA_IN_STREAM_ID, getTag(),
						        attribute.toString());
					}
				}
			});
			...
			HANDLERS.put(Constants.CHARACTERISTICS, new AttributeParser<MediaData.Builder>() {
				...
				public void parse(Attribute attribute, MediaData.Builder builder, ParseState state)
				        throws ParseException {
					final String[] characteristicStrings = ParseUtil.parseQuotedString(attribute.value, getTag())
					        .split(Constants.COMMA);

					if (characteristicStrings.length == 0) {
						throw ParseException.create(ParseExceptionType.EMPTY_MEDIA_CHARACTERISTICS, getTag(),
						        attribute.toString());
					} else {
			...
			HANDLERS.put(Constants.CHANNELS, new AttributeParser<MediaData.Builder>() {
				...
				public void parse(Attribute attribute, MediaData.Builder builder, ParseState state)
				        throws ParseException {
					final String[] channelsStrings = ParseUtil.parseQuotedString(attribute.value, getTag())
					        .split(Constants.LIST_SEPARATOR);

					if (channelsStrings.length == 0 || channelsStrings[0].isEmpty()) {
						throw ParseException.create(ParseExceptionType.EMPTY_MEDIA_CHANNELS, getTag(),
						        attribute.toString());
					} else {
	...
	static final IExtTagParser EXT_X_I_FRAME_STREAM_INF = new IExtTagParser() {
		private final LineParser mLineParser = new MasterPlaylistLineParser(this);
		private final Map<String, AttributeParser<IFrameStreamInfo.Builder>> HANDLERS = makeExtStreamInfHandlers(
		        getTag());
	...
	static <T extends StreamInfoBuilder> Map<String, AttributeParser<T>> makeExtStreamInfHandlers(final String tag) {
		...
		handlers.put(Constants.CODECS, new AttributeParser<T>() {
			@Override
			public void parse(Attribute attribute, T builder, ParseState state) throws ParseException {
				final String[] characteristicStrings = ParseUtil.parseQuotedString(attribute.value, tag)
				        .split(Constants.COMMA);
