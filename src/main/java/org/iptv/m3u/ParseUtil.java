...
import org.iptv.m3u.data.ByteRange;
import org.iptv.m3u.data.Resolution;
...
final class ParseUtil {
	...
	public static <T> void parseAttributes(String line, T builder, ParseState state,
	        Map<String, ? extends AttributeParser<T>> handlers, String tag) throws ParseException {
		...
		for (String string : splitAttributeList(line, tag)) {
			...
			if (separator == -1 || (quote != -1 && quote < separator)) {
				...
				if (!attributeNames.add(name)) {
					throw ParseException.create(ParseExceptionType.MULTIPLE_ATTRIBUTE_NAME_INSTANCES, tag,
					        attributes.toString());
				}
